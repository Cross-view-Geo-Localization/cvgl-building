...
def get_drone_data(path):
    ...
    for class_name in os.listdir(path):
        ...
        for scale_name in os.listdir(class_path):
            scale_path = os.path.join(class_path, scale_name)
            
            if not os.path.isdir(scale_path):
                continue
                
            for file_name in os.listdir(scale_path):
                if file_name.lower().endswith(('.png', '.jpg', '.jpeg', '.tif')):                   
                    relative_file_path = os.path.join(scale_name, file_name)
                    data[class_name]["files"].append(relative_file_path)
                    
    return data

def get_satellite_data(path):
    data = {}
...
class SUES200DatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if mode == "drone":
            self.data_dict = get_drone_data(data_folder)
        elif mode == "satellite":
            self.data_dict = get_satellite_data(data_folder)
