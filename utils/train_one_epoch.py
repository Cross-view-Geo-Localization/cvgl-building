...
def train_one_epoch_with_labels(config, model, dataloader, loss_fn, optimizer, scheduler=None, scaler=None):
    """Single-epoch training loop."""
    model.train()
    losses = []
    ...
    for query, reference, _ids, labels in bar:
        ...
        if scaler is not None:
            ...
            with autocast(device_type="cuda", enabled=config.training.mixed_precision):
                feat_q, feat_r = model(query, reference)
                loss = loss_fn(feat_q, feat_r, labels, model.logit_scale.exp())
