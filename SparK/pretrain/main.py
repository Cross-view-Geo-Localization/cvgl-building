...
def main_pt():
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            misc.save_checkpoint_with_meta_info_and_opt_state(f'{args.model}_withdecoder.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
            misc.save_checkpoint_model_weights_only(f'{args.model}_without_decoder.pth', args, model_without_ddp.sparse_encoder.sp_cnn.state_dict())
